...
class MVBGE(nn.Module):
    """
        MVBGE Module layer, modified from the given VBGE, in terms of each layer's aggregation step
    """

    def __init__(self, opt):
        super(MVBGE, self).__init__()
        self.opt = opt
        self.layer_number = opt["GNN"]
        self.encoder = []
        for i in range(self.layer_number - 1):
            self.encoder.append(MDGCNLayer(opt))
        self.encoder.append(MLastLayer(opt))
...
class MDGCNLayer(nn.Module):
    """
        MDGCN Module layer
    """

    def __init__(self, opt):
        super(MDGCNLayer, self).__init__()
        self.opt = opt
        self.dropout = opt["dropout"]
        self.gc1 = GCN(
        ...
        # for logs_1
        self.user_union = nn.Linear(2*opt["feature_dim"], opt["feature_dim"])
        self.item_union = nn.Linear(2*opt["feature_dim"], opt["feature_dim"])
        self.inter = nn.Linear(opt["feature_dim"], opt["feature_dim"])

    def forward(self, ufea, vfea, UV_adj, VU_adj):
        User_ho0 = self.gc1(ufea, VU_adj)
        Item_ho0 = self.gc2(vfea, UV_adj)
        # modified here to also include interaction
        User_ho = self.gc3(User_ho0, UV_adj)
        Item_ho = self.gc4(Item_ho0, VU_adj)
        User_co = self.inter(Item_ho0)
        Item_co = self.inter(User_ho0)
        User = torch.cat((User_ho+User_co, ufea), dim=1)
        Item = torch.cat((Item_ho+Item_co, vfea), dim=1)
        User = self.user_union(User)
        Item = self.item_union(Item)
        return F.relu(User), F.relu(Item)

    # for logs
    #     self.user_union = nn.Linear(3*opt["feature_dim"], opt["feature_dim"])
    #     self.item_union = nn.Linear(3*opt["feature_dim"], opt["feature_dim"])
    ...
    #     User_ho = self.gc3(User_ho0, UV_adj)
    #     Item_ho = self.gc4(Item_ho0, VU_adj)
    #     User = torch.cat((User_ho, Item_ho0, ufea), dim=1)
    #     Item = torch.cat((Item_ho, User_ho0, vfea), dim=1)
    #     User = self.user_union(User)
    #     Item = self.item_union(Item)
    #     return F.relu(User), F.relu(Item)
...
class MLastLayer(nn.Module):
    """
        DGCN Module layer
    """

    def __init__(self, opt):
        super(MLastLayer, self).__init__()
        self.opt = opt
        self.dropout = opt["dropout"]
        self.gc1 = GCN(
            nfeat=opt["feature_dim"],
